...
NAN_MODULE_INIT(SVM::Init) {
  ...
  ctor->SetClassName(Nan::New("SVM").ToLocalChecked());
	...
	Nan::SetAccessor(ctor->InstanceTemplate(), FF_V8STRING("c"), c);
	Nan::SetAccessor(ctor->InstanceTemplate(), FF_V8STRING("coef0"), coef0);
	Nan::SetAccessor(ctor->InstanceTemplate(), FF_V8STRING("degree"), degree);
	Nan::SetAccessor(ctor->InstanceTemplate(), FF_V8STRING("nu"), nu);
	Nan::SetAccessor(ctor->InstanceTemplate(), FF_V8STRING("p"), p);
	Nan::SetAccessor(ctor->InstanceTemplate(), FF_V8STRING("kernelType"), kernelType);
	Nan::SetAccessor(ctor->InstanceTemplate(), FF_V8STRING("classWeights"), classWeights);
...
NAN_METHOD(SVM::Train) {
	...
	if (FF_IS_INSTANCE(Mat::constructor, info[0])) {
		FF_ARG_OBJ(0, cv::Ptr<cv::ml::TrainData> trainData, TrainData::constructor, FF_UNWRAP_TRAINDATA_AND_GET);
...
NAN_METHOD(SVM::TrainAuto) {
	...
	FF_GET_OBJ_IFDEF(optArgs, cv::ml::ParamGrid cGrid, "cGrid", ParamGrid::constructor, FF_UNWRAP_PARAMGRID_AND_GET, ParamGrid, cv::ml::SVM::getDefaultGrid(cv::ml::SVM::C));
	FF_GET_OBJ_IFDEF(optArgs, cv::ml::ParamGrid gammaGrid, "gammaGrid", ParamGrid::constructor, FF_UNWRAP_PARAMGRID_AND_GET, ParamGrid, cv::ml::SVM::getDefaultGrid(cv::ml::SVM::GAMMA));
	FF_GET_OBJ_IFDEF(optArgs, cv::ml::ParamGrid pGrid, "pGrid", ParamGrid::constructor, FF_UNWRAP_PARAMGRID_AND_GET, ParamGrid, cv::ml::SVM::getDefaultGrid(cv::ml::SVM::P));
	FF_GET_OBJ_IFDEF(optArgs, cv::ml::ParamGrid nuGrid, "nuGrid", ParamGrid::constructor, FF_UNWRAP_PARAMGRID_AND_GET, ParamGrid, cv::ml::SVM::getDefaultGrid(cv::ml::SVM::NU));
	FF_GET_OBJ_IFDEF(optArgs, cv::ml::ParamGrid coeffGrid, "coeffGrid", ParamGrid::constructor, FF_UNWRAP_PARAMGRID_AND_GET, ParamGrid, cv::ml::SVM::getDefaultGrid(cv::ml::SVM::COEF));
	FF_GET_OBJ_IFDEF(optArgs, cv::ml::ParamGrid degreeGrid, "degreeGrid", ParamGrid::constructor, FF_UNWRAP_PARAMGRID_AND_GET, ParamGrid, cv::ml::SVM::getDefaultGrid(cv::ml::SVM::DEGREE));
	FF_GET_BOOL_IFDEF(optArgs, bool balanced, "balanced", false);

	FF_ARG_UINT_IFDEF(1, kFold, kFold);
	FF_ARG_OBJ_IFDEF(2, cGrid, ParamGrid::constructor, FF_UNWRAP_PARAMGRID_AND_GET, cGrid);
	FF_ARG_OBJ_IFDEF(3, gammaGrid, ParamGrid::constructor, FF_UNWRAP_PARAMGRID_AND_GET, gammaGrid);
	FF_ARG_OBJ_IFDEF(4, pGrid, ParamGrid::constructor, FF_UNWRAP_PARAMGRID_AND_GET, pGrid);
	FF_ARG_OBJ_IFDEF(5, nuGrid, ParamGrid::constructor, FF_UNWRAP_PARAMGRID_AND_GET, nuGrid);
	FF_ARG_OBJ_IFDEF(6, coeffGrid, ParamGrid::constructor, FF_UNWRAP_PARAMGRID_AND_GET, coeffGrid);
	FF_ARG_OBJ_IFDEF(7, degreeGrid, ParamGrid::constructor, FF_UNWRAP_PARAMGRID_AND_GET, degreeGrid);
...
NAN_METHOD(SVM::PredictWithResults) {
	...
	FF_OBJ jsResults = FF_NEW(Mat::constructor);
	float probability = FF_UNWRAP(info.This(), SVM)->svm->predict(samples, FF_UNWRAP_MAT_AND_GET(jsResults), flags);
	FF_OBJ ret = FF_NEW_OBJ();
	ret->Set(FF_V8STRING("results"), jsResults);
	ret->Set(FF_V8STRING("probability"), Nan::New(probability));
	info.GetReturnValue().Set(ret);
}
...
void SVM::setParams(v8::Local<v8::Object> params) {
	...
	FF_GET_OBJ_IFDEF(params, cv::Mat classWeights, "classWeights", Mat::constructor, FF_UNWRAP_MAT_AND_GET, Mat, this->svm->getClassWeights());
