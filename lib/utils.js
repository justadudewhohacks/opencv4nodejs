const fs = require('fs');
const path = require('path');

const resolveFile = (filePath, file) => (file ? path.resolve(filePath, file) : path.resolve(filePath)).replace(/\\/g, '/');

const modules = [
  'core',
  'highgui',
  'imgcodecs',
  'imgproc',
  'features2d',
  'calib3d',
  'photo',
  'video',
  'videoio',
  'videostab'
];

const xmodules = [
  'xfeatures2d',
  'ximgproc'
];

const opencvDir = process.env.OPENCV_DIR;
let includeDir = process.env.OPENCV_INCLUDE_DIR;

const libSuffix = process.platform === 'win32' ? '.lib' : '.so';
const libDir = process.env.OPENCV_LIB_DIR;

if (!opencvDir && !includeDir) {
  throw new Error('not defined: process.env.OPENCV_DIR or process.env.OPENCV_INCLUDE_DIR');
}

if (!libDir) {
  throw new Error('process.env.OPENCV_LIB_DIR not defined');
}

if (!includeDir) {
  includeDir = resolveFile(opencvDir, 'include');
}

const opencv2Include = resolveFile(includeDir, 'opencv2');

const getModulesHaveHeader = () => new Promise((resolve) => {
  fs.readdir(opencv2Include, (err, files) => {
    if (err) throw(err);
    const foundHeaders = [];
    modules.forEach(m => {
      if (!files.some(file => file.includes(`${m}.h`))) {
        throw new Error(`'${m}.h' not found in ${opencv2Include}`);
      }
      foundHeaders.push(m);
    });
    xmodules.forEach(m => {
      if (files.some(file => file.includes(`${m}.h`))) {
        foundHeaders.push(m);
      }
    });
    resolve(foundHeaders);
  });
});

const getLibs = () => new Promise((resolve) => {
  fs.readdir(resolveFile(libDir), (err, files) => {
    if (err) throw(err);
    const libsInDir = files.filter(f => f.includes(libSuffix));

    // if modules bundled in single lib file
    const world = libsInDir.find(file => file.includes('world'));
    if (world) {
      return resolve([{
        module: 'world',
        lib: resolveFile(libDir, world)
      }]);
    }

    if (!libsInDir.length) {
      throw new Error(`no valid '${libSuffix}' files found in in OPENCV_LIB_DIR ('${libDir}')`);
    }

    const libs = [];
    modules.forEach(m => {
      const lib = libsInDir.find(file => file.includes(m));
      if (!lib) {
        throw new Error(`lib for module '${m}' not found in OPENCV_LIB_DIR ('${libDir}')`);
      }
      libs.push({
        module: m,
        lib: resolveFile(libDir, lib)
      });
    });
    xmodules.forEach(m => {
      const lib = libsInDir.find(file => file.includes(m));
      if (lib) {
        libs.push({
          module: m,
          lib: resolveFile(libDir, lib)
        });
      }
    })
    resolve(libs);
  });
});

module.exports = {
  modules,
  xmodules,
  includeDir,
  getModulesHaveHeader,
  getLibs
};
