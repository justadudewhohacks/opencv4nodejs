...
function getOpenCV(opt) {
    ...
    Object.keys(haarCascades).forEach(key => opencvBuild[key] = (0, commons_1.resolvePath)(path.join(__dirname, 'haarcascades'), haarCascades[key]));
    Object.keys(lbpCascades).forEach(key => opencvBuild[key] = (0, commons_1.resolvePath)(path.join(__dirname, 'lbpcascades'), lbpCascades[key]));
    return opencvBuild;
}
