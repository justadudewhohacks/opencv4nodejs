...
export default function (args: TestContext) {
  ...
  describe('fitLine', () => {
    ...
    it('should return lineParams for 3D points', () => {
      const points3D = [new cv.Point3(0, 0, 0), new cv.Point3(10, 10, 10)];
    ...
    it('should return lineParams for 3D fp points', () => {
      const points3D = [new cv.Point3(0, 0, 0), new cv.Point3(10.9, 10.1, 10.5)];
      const lineParams = cv.fitLine(points3D, distType, param, reps, aeps);
      expect(lineParams).to.be.an('array').lengthOf(6);
      expect(lineParams).to.not.have.members(Array(6).fill(0));
    });
  ...
  describe('transformation matrix getters', () => {
    const srcPoints = [new cv.Point2(0, 0), new cv.Point2(10, 10), new cv.Point2(0, 10)];
    const dstPoints = [new cv.Point2(0, 0), new cv.Point2(20, 20), new cv.Point2(0, 20)];
