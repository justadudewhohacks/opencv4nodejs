...
module.exports = () => {
  ...
  describe('rectify', () => {
    ...
    ((!process.env.DOCKER_BUILD && !process.env.BINDINGS_DEBUG && global.cvVersionLowerThan(3, 3, 0)) ? describe : describe.skip)('rectify3Collinear', () => {
      const alpha = 0;
  ...
  (global.cvVersionLowerThan(3, 1, 0) ? describe.skip : describe)('findEssentialMat', () => {
    const expectOutput = (res) => {
  ...
  (global.cvVersionLowerThan(3, 1, 0) ? describe.skip : describe)('recoverPose', () => {
    const expectOutput = (res) => {
      expect(res).to.have.property('returnValue').to.be.a('Number');
